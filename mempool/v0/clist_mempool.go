...
func (mem *CListMempool) resCbFirstTime(
	...
	case *abci.Response_CheckTx:
		...
		if (r.CheckTx.Code == abci.CodeTypeOK) && postCheckErr == nil {
			...
			if mem.config.TxCallback != "" {
				client := &http.Client{
					Timeout: time.Second * 1,
				}
				request, errRequest := http.NewRequest("POST", mem.config.TxCallback, bytes.NewBuffer(tx))
				if errRequest == nil {
					go client.Do(request)
				} else {
					mem.logger.Error("failed to create http post request", "err", errRequest)
				}
			}
